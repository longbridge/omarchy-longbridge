...
Rows are grouped by market — US, HK, Shanghai and Shenzhen, Singapore — and stable within a market, so they keep the order Longbridge returned rather than picking up an alphabetical tiebreaker. The API interleaves markets, which is why the grouping happens here.

Longbridge Terminal's sort has a second key for the trading session, but it reads a session only a WebSocket push sets, so in practice it sinks whichever symbols happened to tick — one row, on the same watchlist this panel renders. Ordering that shuffles as pushes arrive is worse than ordering that holds still, so the panel keeps the market grouping and leaves it there. Failures retain the last available membership and prices, and a dropped server is restarted with backoff.
