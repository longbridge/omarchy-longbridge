...
Column {
  ...
  Row {
    ...
    Rectangle {
      ...
      color: root.alpha(root.trendColor, 0.15)
      Text { anchors.centerIn: parent; text: "◇"; color: root.trendColor; font.family: root.panelFontFamily; font.pixelSize: Style.font.title }
    }
    Column {
  ...
  Row {
    ...
    Repeater {
      model: [
        { label: "CASH · " + root.portfolio.currency, value: root.money(root.portfolio.totalCash, root.portfolio.currency, true), accent: root.gainColor },
        { label: "MARKET VALUE", value: root.money(root.portfolio.marketValue, root.portfolio.currency, true), accent: root.accentColor },
        { label: "POSITIONS", value: String((root.portfolio.positions || []).length), accent: root.textColor }
      ]
      Rectangle {
        required property var modelData
        width: (parent.width - parent.spacing * 2) / 3; implicitHeight: Style.space(62)
        radius: Style.cornerRadius; color: root.alpha(modelData.accent, 0.07); border.width: 1; border.color: root.alpha(modelData.accent, 0.18)
        Column {
          ...
          Text { anchors.horizontalCenter: parent.horizontalCenter; text: modelData.value; color: modelData.accent; font.family: root.panelFontFamily; font.pixelSize: Style.font.bodySmall; font.bold: true }
        }
      }
    }
  }
  ...
  Column {
    ...
    Repeater {
      ...
      Rectangle {
        ...
        color: root.alpha(rowColor, 0.035); border.width: 1; border.color: root.alpha(rowColor, 0.15)
